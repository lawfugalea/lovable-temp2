...
export function FinanceTransactionEditor({ householdId, transaction, onClose, onSaved, onError, onNotice }: EditorProps) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/45 p-4" role="dialog" aria-modal="true"><Card className="max-h-[90vh] w-full max-w-xl overflow-auto hover:-translate-y-0"><CardHeader><div className="flex items-start justify-between gap-3"><div><CardTitle>Improve this transaction</CardTitle><CardDescription>Correct the merchant or category and optionally teach HouseFlow the pattern.</CardDescription></div><Button size="icon" variant="ghost" onClick={onClose}><X className="h-4 w-4" /></Button></div></CardHeader><CardContent className="space-y-4"><div><label className="mb-1.5 block text-sm font-medium">Friendly merchant name</label><Input value={merchantName} onChange={event => setMerchantName(event.target.value)} maxLength={160} /></div><div><label className="mb-1.5 block text-sm font-medium">Category</label><select className="h-10 w-full rounded-lg border border-cozy-gray-300 bg-white px-3 text-sm" value={category} onChange={event => setCategory(event.target.value)}>{FINANCE_CATEGORIES.map(item => <option key={item}>{item}</option>)}</select></div><label className="flex items-start gap-3 rounded-xl border border-cozy-gray-200 p-3"><input type="checkbox" className="mt-1" checked={applyToSimilar} onChange={event => setApplyToSimilar(event.target.checked)} /><span><span className="block text-sm font-medium">Apply to matching transactions</span><span className="block text-xs text-cozy-text-muted">Creates a reusable local rule for past and future activity.</span></span></label>{applyToSimilar && <div className="space-y-3 rounded-xl bg-cozy-warm/60 p-4"><div className="grid gap-3 sm:grid-cols-2"><div><label className="mb-1 block text-xs font-medium">Match style</label><select className="h-9 w-full rounded-lg border border-cozy-gray-300 bg-white px-3 text-sm" value={matchType} onChange={event => setMatchType(event.target.value as 'EXACT' | 'CONTAINS')}><option value="EXACT">Same normalized merchant</option><option value="CONTAINS">Merchant contains pattern</option></select></div><label className="flex items-center gap-2 pt-5 text-sm"><input type="checkbox" checked={allAccounts} onChange={event => setAllAccounts(event.target.checked)} />Use on all my accounts</label></div><div className="text-sm">{loading ? <span className="flex items-center text-cozy-text-muted"><Loader2 className="mr-2 h-4 w-4 animate-spin" />Checking matches…</span> : <><strong>{preview?.count || 0}</strong> transaction{preview?.count === 1 ? '' : 's'} match this pattern.{preview?.samples?.length ? <p className="mt-1 truncate text-xs text-cozy-text-muted">Examples: {[...new Set(preview.samples.map(item => item.merchantName))].slice(0, 3).join(', ')}</p> : null}</>}</div></div>}<div className="flex flex-wrap justify-between gap-2 border-t border-cozy-gray-200 pt-4"><div>{transaction.enrichmentSource === 'override' && <Button variant="ghost" onClick={resetOverride} disabled={saving}><RotateCcw className="mr-2 h-4 w-4" />Reset single edit</Button>}</div><div className="flex gap-2"><Button variant="ghost" onClick={onClose}>Cancel</Button><Button onClick={save} disabled={!merchantName.trim() || saving || (applyToSimilar && loading)}>{saving && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Save</Button></div></div></CardContent></Card></div>
}
...
export function FinanceRulesPanel({ householdId, reloadKey, onChanged, onError, onNotice }: RulesProps) {
  ...
  return <Card className="hover:-translate-y-0"><CardHeader><button type="button" className="flex w-full items-center justify-between gap-3 text-left" onClick={() => setOpen(value => !value)}><div><div className="flex items-center gap-2"><SlidersHorizontal className="h-5 w-5 text-cozy-primary" /><CardTitle className="text-lg">Learned merchant rules</CardTitle></div><CardDescription>Review the corrections HouseFlow applies locally.</CardDescription></div><Badge variant="outline">{open ? 'Hide' : 'Manage'}</Badge></button></CardHeader>{open && <CardContent>{loading ? <div className="flex items-center justify-center py-8 text-sm text-cozy-text-muted"><Loader2 className="mr-2 h-4 w-4 animate-spin" />Loading rules…</div> : rules.length === 0 ? <p className="py-6 text-center text-sm text-cozy-text-muted">No learned rules yet. Use the edit button on a transaction to create one.</p> : <div className="divide-y divide-cozy-gray-200">{rules.map(rule => <div key={rule.id} className="flex flex-col gap-3 py-3 sm:flex-row sm:items-center sm:justify-between"><div><div className="flex flex-wrap items-center gap-2"><p className="font-medium">{rule.merchantName || rule.matchValue}</p>{rule.category && <Badge variant="outline">{rule.category}</Badge>}{!rule.enabled && <Badge variant="outline">Paused</Badge>}</div><p className="mt-1 text-xs text-cozy-text-muted">{rule.matchType === 'EXACT' ? 'Exact normalized match' : 'Contains'}: “{rule.matchValue}” · {rule.account?.displayName || 'All accounts'}</p></div><div className="flex gap-1"><Button size="sm" variant="ghost" onClick={() => mutate(rule, 'PATCH', { enabled: !rule.enabled })}><Power className="mr-1 h-3 w-3" />{rule.enabled ? 'Pause' : 'Enable'}</Button><Button size="icon" variant="ghost" aria-label="Delete rule" onClick={() => mutate(rule, 'DELETE')}><Trash2 className="h-4 w-4 text-red-600" /></Button></div></div>)}</div>}</CardContent>}</Card>
}
