...
export default function FinanceCoachPanel({ householdId, accountId, accounts, canManage, reloadKey, onError, onNotice }: Props) {
  ...
  return <section className="space-y-5">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><div className="flex items-center gap-2 text-sm font-medium text-primary"><BrainCircuit className="h-4 w-4" /> Local pattern recognition</div><h2 className="mt-1 text-2xl font-bold">Spending coach</h2><p className="mt-1 text-sm text-muted-foreground">Neutral, explainable observations — never a judgement or financial advice.</p></div>{canManage && <div className="flex gap-2"><Button variant="outline" onClick={() => setShowLimit(value => !value)}><Target className="mr-2 h-4 w-4" />Set a limit</Button><Button onClick={openAiPreview} disabled={action === 'preview'}>{action === 'preview' ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Sparkles className="mr-2 h-4 w-4" />}Improve with AI</Button></div>}</div>

    {showLimit && <Card className="hover:-translate-y-0"><CardHeader><CardTitle className="text-lg">New monthly limit</CardTitle><CardDescription>Track a category or one merchant across all accounts, or select a specific account.</CardDescription></CardHeader><CardContent className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-5"><select aria-label="Limit account" className="h-10 rounded-lg border border-input bg-card px-3 text-sm" value={limitForm.accountId} onChange={event => setLimitForm({ ...limitForm, accountId: event.target.value })}><option value="">All accounts</option>{accounts.map(item => <option key={item.id} value={item.id}>{item.displayName}</option>)}</select><select aria-label="Limit scope" className="h-10 rounded-lg border border-input bg-card px-3 text-sm" value={limitForm.scope} onChange={event => setLimitForm({ ...limitForm, scope: event.target.value, displayName: event.target.value === 'CATEGORY' ? 'Dining' : '' })}><option value="CATEGORY">Category</option><option value="MERCHANT">Merchant</option></select>{limitForm.scope === 'CATEGORY' ? <select aria-label="Limit category" className="h-10 rounded-lg border border-input bg-card px-3 text-sm" value={limitForm.displayName} onChange={event => setLimitForm({ ...limitForm, displayName: event.target.value })}>{FINANCE_CATEGORIES.filter(item => !['Income', 'Refunds', 'Transfers'].includes(item)).map(item => <option key={item}>{item}</option>)}</select> : <Input placeholder="Merchant name" value={limitForm.displayName} onChange={event => setLimitForm({ ...limitForm, displayName: event.target.value })} />}<Input type="number" min="1" step="1" placeholder="Monthly amount" value={limitForm.amount} onChange={event => setLimitForm({ ...limitForm, amount: event.target.value })} /><Button onClick={addLimit} disabled={!limitForm.displayName || !limitForm.amount || action === 'limit'}>{action === 'limit' && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Add limit</Button></CardContent></Card>}

    {limits.length > 0 && <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">{limits.map(limit => <Card key={limit.id} className={`hover:-translate-y-0 ${limit.exceeded ? 'border-red-200' : ''}`}><CardContent className="p-5"><div className="flex items-start justify-between gap-3"><div><div className="flex items-center gap-2"><Gauge className="h-4 w-4 text-primary" /><h3 className="font-semibold">{limit.displayName}</h3><Badge variant="outline">Monthly</Badge></div><p className="mt-1 text-xs text-muted-foreground">{money(limit.spent, limit.currency)} of {money(limit.amount, limit.currency)} · projected {money(limit.projected, limit.currency)}</p></div>{canManage && <Button size="icon" variant="ghost" aria-label="Delete limit" onClick={() => deleteLimit(limit.id)} disabled={action === `delete:${limit.id}`}><Trash2 className="h-4 w-4 text-red-600" /></Button>}</div><div className="mt-4 h-3 overflow-hidden rounded-full bg-muted"><div className={`h-full rounded-full ${limit.exceeded ? 'bg-red-500' : limit.percentage >= 80 ? 'bg-amber-500' : 'bg-primary'}`} style={{ width: `${Math.min(100, Math.max(2, limit.percentage))}%` }} /></div><p className={`mt-2 text-sm font-medium ${limit.exceeded ? 'text-red-700' : 'text-foreground'}`}>{limit.percentage.toFixed(0)}% used{limit.exceeded ? ' · limit exceeded' : ''}</p></CardContent></Card>)}</div>}

    <div><h3 className="text-lg font-semibold">Patterns worth a look</h3><p className="text-sm text-muted-foreground">Bills, transfers, cash, income, and confirmed subscriptions are excluded by default.</p></div>
    {loading ? <Card><CardContent className="flex min-h-48 items-center justify-center text-sm text-muted-foreground"><Loader2 className="mr-2 h-5 w-5 animate-spin" />Building local patterns…</CardContent></Card> : signals.length === 0 ? <Card><CardContent className="py-12 text-center"><ShieldCheck className="mx-auto mb-3 h-8 w-8 text-primary" /><p className="font-medium">No notable discretionary patterns right now</p><p className="mt-1 text-sm text-muted-foreground">This is not a score. New observations appear only when a clear threshold is crossed.</p></CardContent></Card> : <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">{signals.map(signal => <Card key={signal.key} className="hover:-translate-y-0"><CardContent className="p-5"><div className="flex items-start justify-between gap-3"><Badge variant="outline">{signalLabel(signal.kind)}</Badge><span className="text-xs text-muted-foreground">{signal.transactionIds.length} matching transactions</span></div><h3 className="mt-3 font-semibold">{signal.title}</h3><p className="mt-2 text-sm text-muted-foreground">{signal.explanation}</p><div className="mt-3 flex gap-2 rounded-lg bg-primary/10 p-3 text-sm text-foreground"><ChevronRight className="mt-0.5 h-4 w-4 shrink-0 text-primary" />{signal.suggestion}</div>{canManage && <div className="mt-4 flex gap-2"><Button size="sm" variant="outline" onClick={() => feedback(signal.key, 'SNOOZED')}>Snooze 30 days</Button><Button size="sm" variant="ghost" onClick={() => feedback(signal.key, 'DISMISSED')}>Dismiss</Button></div>}</CardContent></Card>)}</div>}

    {(aiResult || canManage) && <Card className="overflow-hidden border-violet-200 hover:-translate-y-0"><CardHeader className="bg-gradient-to-r from-violet-50 to-blue-50"><div className="flex items-start justify-between gap-3"><div><div className="flex items-center gap-2"><Sparkles className="h-5 w-5 text-violet-700" /><CardTitle className="text-lg">Optional AI perspective</CardTitle></div><CardDescription>Suggestions from redacted aggregates; local analysis remains authoritative.</CardDescription></div>{canManage && aiResult && <Button size="sm" variant="ghost" onClick={revokeAi}>Delete & revoke</Button>}</div></CardHeader><CardContent className="p-5">{aiResult ? <><p className="text-sm font-medium">{aiResult.summary}</p><div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2">{aiResult.observations.map((item, index) => <div key={`${item.title}-${index}`} className="rounded-xl border border-border p-4"><div className="flex items-start justify-between gap-2"><h4 className="font-semibold">{item.title}</h4>{item.confidence !== null && <Badge variant="outline">{Math.round(item.confidence * 100)}%</Badge>}</div><p className="mt-2 text-sm text-muted-foreground">{item.explanation}</p><p className="mt-3 text-sm text-foreground">{item.suggestion}</p></div>)}</div>{aiCreatedAt && <p className="mt-4 text-xs text-muted-foreground">Generated {new Intl.DateTimeFormat('en-MT', { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(aiCreatedAt))}</p>}</> : <div className="text-center py-4"><Eye className="mx-auto mb-2 h-7 w-7 text-muted-foreground" /><p className="font-medium">No AI analysis has been requested</p><p className="mt-1 text-sm text-muted-foreground">Nothing leaves Clankeep unless the finance owner previews and sends it.</p></div>}</CardContent></Card>}

    {preview && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/45 p-4" role="dialog" aria-modal="true"><Card className="max-h-[90vh] w-full max-w-3xl overflow-auto hover:-translate-y-0"><CardHeader><div className="flex items-start justify-between gap-3"><div><CardTitle>Review what DeepSeek will receive</CardTitle><CardDescription>No account IDs, exact dates, bank notes, references, or personal identity are included.</CardDescription></div><Button size="icon" variant="ghost" onClick={() => setPreview(null)}><X className="h-4 w-4" /></Button></div></CardHeader><CardContent><div className={`rounded-lg border p-3 text-sm ${preview.configured ? 'border-green-200 bg-green-50 text-green-800 dark:border-green-900 dark:bg-green-950/40 dark:text-green-300' : 'border-amber-200 bg-amber-50 text-amber-800 dark:border-amber-900 dark:bg-amber-950/40 dark:text-amber-200'}`}>{preview.configured ? 'DeepSeek is configured on the server.' : 'Add DEEPSEEK_API_KEY to the server before requesting analysis.'}</div><pre className="mt-4 max-h-80 overflow-auto rounded-xl bg-slate-950 p-4 text-xs text-slate-100">{JSON.stringify(preview.payload, null, 2)}</pre>{!preview.consented && <label className="mt-4 flex items-start gap-3 rounded-lg border border-border p-3 text-sm"><input type="checkbox" className="mt-1" checked={consent} onChange={event => setConsent(event.target.checked)} /><span>I explicitly consent to sending this redacted payload to DeepSeek for this optional analysis. I understand DeepSeek states that data is processed in China and may be used to improve its services.</span></label>}<div className="mt-5 flex justify-end gap-2"><Button variant="ghost" onClick={() => setPreview(null)}>Cancel</Button><Button onClick={runAi} disabled={!preview.configured || (!preview.consented && !consent) || action === 'analyze'}>{action === 'analyze' ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Sparkles className="mr-2 h-4 w-4" />}Send redacted analysis</Button></div></CardContent></Card></div>}
  </section>
}
