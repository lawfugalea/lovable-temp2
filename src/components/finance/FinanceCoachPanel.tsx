...
export default function FinanceCoachPanel({ householdId, accountId, accounts, canManage, reloadKey, onError, onNotice }: Props) {
  ...
  return <section className="space-y-5">
    ...
    {showLimit && <Card className="hover:-translate-y-0"><CardHeader><CardTitle className="text-lg">New monthly limit</CardTitle><CardDescription>Track a category or one merchant across all accounts, or select a specific account.</CardDescription></CardHeader><CardContent className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-5"><select className="h-10 rounded-lg border border-input bg-white px-3 text-sm" value={limitForm.accountId} onChange={event => setLimitForm({ ...limitForm, accountId: event.target.value })}><option value="">All accounts</option>{accounts.map(item => <option key={item.id} value={item.id}>{item.displayName}</option>)}</select><select className="h-10 rounded-lg border border-input bg-white px-3 text-sm" value={limitForm.scope} onChange={event => setLimitForm({ ...limitForm, scope: event.target.value, displayName: event.target.value === 'CATEGORY' ? 'Dining' : '' })}><option value="CATEGORY">Category</option><option value="MERCHANT">Merchant</option></select>{limitForm.scope === 'CATEGORY' ? <select className="h-10 rounded-lg border border-input bg-white px-3 text-sm" value={limitForm.displayName} onChange={event => setLimitForm({ ...limitForm, displayName: event.target.value })}>{FINANCE_CATEGORIES.filter(item => !['Income', 'Refunds', 'Transfers'].includes(item)).map(item => <option key={item}>{item}</option>)}</select> : <Input placeholder="Merchant name" value={limitForm.displayName} onChange={event => setLimitForm({ ...limitForm, displayName: event.target.value })} />}<Input type="number" min="1" step="1" placeholder="Monthly amount" value={limitForm.amount} onChange={event => setLimitForm({ ...limitForm, amount: event.target.value })} /><Button onClick={addLimit} disabled={!limitForm.displayName || !limitForm.amount || action === 'limit'}>{action === 'limit' && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Add limit</Button></CardContent></Card>}
