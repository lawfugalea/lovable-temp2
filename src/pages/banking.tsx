...
export default function BankingPage() {
  ...
  useEffect(() => {
    ...
    if (router.query.bankConnected || router.query.bankError || router.query.syncWarning) {
      void router.replace('/banking', undefined, { shallow: true })
    }
      ...
      <div className="space-y-6 max-w-7xl mx-auto">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <h1 className="font-display text-3xl font-bold text-foreground">Your connected banking</h1>
            <p className="text-muted-foreground mt-1">Balances, transactions, and bank-powered insights in a separate read-only space.</p>
          </div>
        ...
        {householdId && (
          <div className="flex flex-col gap-3 rounded-xl border border-border bg-card p-2 lg:flex-row lg:items-center lg:justify-between">
            <nav className="grid grid-cols-2 gap-1 sm:flex sm:flex-wrap" aria-label="Banking sections">
              {([
        ...
        {!householdId && (
          <Card>
            <CardContent className="py-12 text-center">
              ...
              <p className="text-sm text-muted-foreground mt-1">Banking visibility follows your active household.</p>
            </CardContent>
          </Card>
        )}

        {householdId && overview && !overview.bankEnabled && (
          <Card>
            <CardContent className="py-12 text-center">
              <Building2 className="mx-auto mb-3 h-10 w-10 text-muted-foreground" />
              <h2 className="text-lg font-semibold">Open Banking is not enabled yet</h2>
              <p className="mx-auto mt-1 max-w-xl text-sm text-muted-foreground">
                This is where connected balances and transactions will live. Your household finance plan remains available under Finance.
              </p>
            </CardContent>
          </Card>
        )}
        ...
        {householdId && overview?.bankEnabled && activeTab === 'overview' && overview.accounts.length === 0 && (
          overview.canManage ? (
            <ModuleFirstRun
              ...
              description="You will continue to BOV to approve read-only access. Clankeep never receives your bank password or permission to make payments."
              action={
                <Button
            ...
            // Someone else owns the connection — nothing for this member to do,
            // so this is a waiting state rather than a first run.
            <EmptyState
              module="banking"
              icon={Building2}
              title="No accounts are shared yet"
              description="The finance owner controls which accounts the household can view."
            />
          )
        )}
