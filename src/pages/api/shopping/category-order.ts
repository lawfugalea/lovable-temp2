...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const list = listId ? await prisma.shoppingList.findUnique({ where: { id: listId }, select: { householdId: true } }) : null; if (!list) return res.status(404).json({ error: 'Shopping list not found' })
  const membership = await prisma.membership.findUnique({ where: { userId_householdId: { userId, householdId: list.householdId } }, select: { id: true } }); if (!membership) return res.status(403).json({ error: 'Forbidden' }); res.setHeader('Cache-Control', 'no-store')
