...
function cleanQty(value: unknown): string | null {
...
function cleanCount(value: unknown): number | null {
  return Number.isInteger(value) && Number(value) >= 1 && Number(value) <= 999 ? Number(value) : null
}

function afterValue(raw: Record<string, unknown>) {
  const title = cleanText(raw.title, 200)
  const quantityCount = cleanCount(raw.quantityCount)
  const category = isShoppingCategory(raw.category) ? raw.category : null
  if (!title || quantityCount === null || !category) return null
  return { title, qty: cleanQty(raw.qty), quantityCount, category }
}

/** Convert provider output into bounded, grounded operations. Invalid changes are dropped. */
export function parseShoppingAiResult(
  parsed: unknown,
  items: ShoppingAiSourceItem[],
  mealIngredients: ShoppingAiMealIngredient[],
): { summary: string; operations: ShoppingAiOperation[] } {
  ...
  const operations: ShoppingAiOperation[] = []

  for (const value of changes.slice(0, 200)) {
    if (!value || typeof value !== 'object') continue
    const raw = value as Record<string, unknown>
    const kind = raw.kind
    const reason = cleanText(raw.reason, 240) || 'Makes the list easier to shop.'

    if (kind === 'update') {
      const ref = typeof raw.itemRef === 'string' ? raw.itemRef : ''
      const item = itemByRef.get(ref)
      if (!item || touchedItems.has(item.id)) continue
      const after = afterValue(raw)
      if (!after) continue
      ...
      operations.push({ id: `op-${operations.length + 1}`, kind: 'update', itemId: item.id, before, after, reason })
      continue
    }
    ...
    if (kind === 'merge') {
      ...
      const products = new Set(candidates.map(item => item.canonicalProductId).filter(Boolean))
      const stores = new Set(candidates.map(item => item.store).filter(Boolean))
      if (products.size > 1 || stores.size > 1) continue
      const titles = new Set(candidates.map(item => normalizeCatalogText(item.title)))
      const quantities = new Set(candidates.map(item => item.qty?.trim().toLowerCase()).filter(Boolean))
      if (products.size === 0 && titles.size > 1) continue
      if (quantities.size > 1) continue
      const keep = candidates.find(item => item.canonicalProductId) || candidates[0]
      const after = afterValue(raw)
      if (!after) continue
      const total = candidates.reduce((sum, item) => sum + item.quantityCount, 0)
      after.quantityCount = Math.min(999, Math.max(after.quantityCount, total))
      candidates.forEach(item => touchedItems.add(item.id))
      operations.push({
        id: `op-${operations.length + 1}`,
        kind: 'merge',
        keepItemId: keep.id,
        removeItemIds: candidates.filter(item => item.id !== keep.id).map(item => item.id),
        before: candidates.map(item => ({ id: item.id, title: item.title, qty: item.qty, quantityCount: item.quantityCount, category: normalizeShoppingItemCategory(item.category) })),
        after,
        reason,
      })
      continue
    ...
    if (kind === 'add') {
      const mealRef = typeof raw.mealRef === 'string' ? raw.mealRef : ''
      const ingredient = mealByRef.get(mealRef)
      if (!ingredient || touchedMeals.has(mealRef) || !isShoppingCategory(raw.category)) continue
      touchedMeals.add(mealRef)
      operations.push({
        id: `op-${operations.length + 1}`,
        kind: 'add',
        mealRef,
...
export async function requestShoppingAi(payload: ShoppingAiPayload): Promise<unknown> {
  ...
  if (!apiKey) throw new Error('AI tidying is not configured')
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(typeof nested?.message === 'string' ? nested.message.slice(0, 300) : 'AI provider request failed')
    }
    const first = body && Array.isArray(body.choices) ? body.choices[0] as Record<string, unknown> | undefined : undefined
    ...
    if (controller.signal.aborted) throw new Error('AI tidying timed out')
    throw error
  } finally {
