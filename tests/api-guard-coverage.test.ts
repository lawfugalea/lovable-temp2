...
const GUARDS = [
  ...
  'getServerSession',
  // The app authenticates with a bearer token instead of a browser session, so
  // it reaches the same authorization decisions through its own two entry
  // points. Both are as load-bearing as the guards above.
  'requireMobileIdentity',
  'requireMobileFinanceAccess',
]
...
const PUBLIC_ROUTES: Record<string, string> = {
  ...
  // The app's token endpoints cannot require an existing session: the credential
  // is the request itself, and each is verified inside the handler.
  'mobile/v1/auth/login.ts': 'Exchanges a password for tokens; rate limited per IP, account, and pair.',
  'mobile/v1/auth/refresh.ts': 'Consumes a rotating refresh token as the credential.',
  'mobile/v1/auth/logout.ts': 'Revokes the presented refresh token; unknown tokens are a no-op.',
}
...
const DELEGATING_ROUTES: Record<string, string> = {
  ...
  // The app reuses the hardened web handlers for the pre-session steps rather
  // than keeping a second copy of the captcha and token rules.
  'mobile/v1/auth/captcha.ts': 'captcha/challenge.ts',
  'mobile/v1/auth/register.ts': 'register.ts',
  'mobile/v1/auth/forgot-password.ts': 'auth/forgot-password.ts',
  'mobile/v1/auth/reset-password.ts': 'auth/reset-password.ts',
}
...
test('every API route reaches an authorization guard or is a reviewed public route', () => {
  ...
  for (const route of routes) {
    ...
    if (route in DELEGATING_ROUTES) {
      ...
      assert.match(
        source,
        // Either a sibling re-export or the `@/pages/api` alias a nested route needs.
        new RegExp(`from\\s+['"](?:\\./|@/pages/api/)${base}['"]`),
