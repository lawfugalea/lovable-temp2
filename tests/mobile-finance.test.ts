...
test('mobile planner routes authenticate with bearer tokens, never browser sessions', () => {
  for (const name of ['index', 'coach', '[kind]']) {
    const route = readFileSync(`src/pages/api/mobile/v1/finance/planner/${name}.ts`, 'utf8')
    assert.match(route, /requireMobileFinanceAccess|requireMobileIdentity/)
